...
export function Greeter(): ReactElement {
  ...
  function handleDeployContract(event: MouseEvent<HTMLButtonElement>) {
    ...
    async function deployGreeterContract(signer: Signer): Promise<void> {
      ...
      try {
        const greeterContract = await Greeter.deploy('Hello HSE!');
  ...
  function handleInstContract(event: MouseEvent<HTMLButtonElement>) {
    ...
    async function deployGreeterContract(signer: Signer): Promise<void> {
      ...
      const Greeter = new ethers.Contract("0x5FbDB2315678afecb367f032d93F642f64180aa3", GreeterArtifact.abi, signer);
      ...
      try {
        ...
        setGreeterContractAddr(Greeter.address);
      } catch (error: any) {
        window.alert(
          'Error!' + (error && error.message ? `\n\n${error.message}` : '')
        );
  ...
  function handleGreetingSubmit(event: MouseEvent<HTMLButtonElement>): void {
    ...
    async function submitGreeting(greeterContract: Contract): Promise<void> {
      try {
        ...
        let tx = await setGreetingTxn.wait();
        ...
        // const _startBlock = 1;
        // const _endBlock = 5000;

        // const filter = greeterContract.filters.GreetedBy("0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266");

        // let filters = greeterContract.filters.GreetedBy("0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266")
        // let events = await greeterContract.queryFilter(filter, _startBlock, _endBlock).then(console.log);
